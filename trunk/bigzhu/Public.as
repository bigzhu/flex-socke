package bigzhu
{
	...
	public class Public extends BaseBig
	{
		//图标
		...
		public static function event(f:Function, ... arg):Function
		{
			var F:Boolean=false;
		...
		public static function eventArgs(f:Function, args:*):Function
		{
			var F:Boolean=false;
			var _f:Function=function(e:*, ..._arg):void
			{
		 ...
		 * 
		 */
		public static function timeCall(time:int, count:int, func:Function, ...arg):*
		{
			var t:Timer = new Timer(time, count);
			t.start();
			t.addEventListener(TimerEvent.TIMER, eventArgs(func, arg));		
		}
		/**
		 *控制什么作为权限树图标显示 
		 * @param item
